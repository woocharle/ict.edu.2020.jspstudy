			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="k" items="${clist}" varStatus="vs">					
						<tr align="center" >
							<td style="padding: 10px">${k.c_num}</td>
							<td>${k.c_name}</td>
							<td>
								정가: ${k.c_price} &nbsp;&nbsp;
							  	<font color="red"> (세일가격: ${k.c_saleprice})</font>
							</td>
							<td>
								<form method="post">
									<input type="number" name="c_quant" value="${k.c_quant}" style=" width:40px"/>
									<input type="button" value="수정" onclick="edit_go(this.form)">
									<input type="hidden" name="idx" value="${k.idx}" />
									<input type="hidden" name="c_id" value="${c_id}">
									<input type="hidden" name="c_num" value="${k.c_quant}" />
							  		<input type="hidden" name="c_saleprice" value="${k.c_saleprice}" />
								</form>
							</td>
							<td>${k.c_total}</td>
							<td>
